...
function fieldInput(field, value, onChange, resource, config) {
  ...
  const options = field === 'interest' ? interests : ['status','stage'].includes(field) ? config.status : field === 'category' ? config.categories : null;
  return <><input type={field.includes('date') ? 'date' : field.includes('time') ? 'time' : field.includes('seconds') || ['reach','views','followers','likes','shares'].includes(field) ? 'number' : 'text'} value={value || ''} onChange={(e) => onChange(field, e.target.value)} list={options ? `${resource}-${field}` : undefined} required={['title','category','platform','name','lead_name','phone_whatsapp','interest','status'].includes(field)} />{options && <datalist id={`${resource}-${field}`}>{options.map((option) => <option key={option} value={option} />)}</datalist>}</>;
}
