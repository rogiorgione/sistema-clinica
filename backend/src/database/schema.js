...
async function initializeDatabase() {
  ...
  await initializeDailyMarketingMachine();
  await seedPremiumOs();
...
async function initializeDailyMarketingMachine() {
  await run(`CREATE TABLE IF NOT EXISTS marketing_goals (id INTEGER PRIMARY KEY AUTOINCREMENT, month TEXT NOT NULL UNIQUE, monthly_budget_target INTEGER NOT NULL DEFAULT 15, notes TEXT, created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP)`);
  await run(`CREATE TABLE IF NOT EXISTS flyer_campaigns (id INTEGER PRIMARY KEY AUTOINCREMENT, code TEXT NOT NULL UNIQUE, name TEXT NOT NULL, neighborhood TEXT, region TEXT, quantity_planned INTEGER NOT NULL DEFAULT 0, responsible TEXT, cost REAL NOT NULL DEFAULT 0, treatment TEXT, offer TEXT, whatsapp_message TEXT, qr_code_payload TEXT, short_link TEXT, status TEXT NOT NULL DEFAULT 'Ativa', created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP)`);
  await run(`CREATE TABLE IF NOT EXISTS flyer_distribution (id INTEGER PRIMARY KEY AUTOINCREMENT, campaign_id INTEGER, neighborhood TEXT, region TEXT, street TEXT, quantity_distributed INTEGER NOT NULL DEFAULT 0, distribution_date TEXT NOT NULL DEFAULT CURRENT_DATE, responsible TEXT, cost REAL NOT NULL DEFAULT 0, notes TEXT, created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (campaign_id) REFERENCES flyer_campaigns(id) ON DELETE SET NULL)`);
  await run(`CREATE TABLE IF NOT EXISTS lead_capture_entries (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL, phone_whatsapp TEXT, interest TEXT, origin TEXT, campaign TEXT, neighborhood TEXT, notes TEXT, urgency TEXT NOT NULL DEFAULT 'Média', best_contact_time TEXT, status TEXT NOT NULL DEFAULT 'Novo lead', qualification TEXT NOT NULL DEFAULT 'Qualificar', temperature TEXT NOT NULL DEFAULT 'Morno', suggested_whatsapp_message TEXT, next_contact_date TEXT, last_interaction_at TEXT, contacted INTEGER NOT NULL DEFAULT 0, created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP)`);
  await run(`CREATE TABLE IF NOT EXISTS commercial_daily_tasks (id INTEGER PRIMARY KEY AUTOINCREMENT, lead_id INTEGER, title TEXT NOT NULL, channel TEXT NOT NULL DEFAULT 'WhatsApp', due_date TEXT NOT NULL DEFAULT CURRENT_DATE, due_time TEXT, status TEXT NOT NULL DEFAULT 'Pendente', priority TEXT NOT NULL DEFAULT 'Média', message TEXT, created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (lead_id) REFERENCES lead_capture_entries(id) ON DELETE SET NULL)`);
  await run(`CREATE TABLE IF NOT EXISTS commercial_followup_actions (id INTEGER PRIMARY KEY AUTOINCREMENT, lead_id INTEGER, task_id INTEGER, action_type TEXT NOT NULL, channel TEXT NOT NULL DEFAULT 'WhatsApp', message TEXT, result TEXT, next_contact_date TEXT, status TEXT NOT NULL DEFAULT 'Registrada', created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (lead_id) REFERENCES lead_capture_entries(id) ON DELETE SET NULL, FOREIGN KEY (task_id) REFERENCES commercial_daily_tasks(id) ON DELETE SET NULL)`);
  await run(`CREATE TABLE IF NOT EXISTS marketing_daily_checklist (id INTEGER PRIMARY KEY AUTOINCREMENT, checklist_date TEXT NOT NULL DEFAULT CURRENT_DATE, item TEXT NOT NULL, category TEXT NOT NULL DEFAULT 'Marketing', planned_quantity INTEGER NOT NULL DEFAULT 1, done_quantity INTEGER NOT NULL DEFAULT 0, status TEXT NOT NULL DEFAULT 'Pendente', UNIQUE(checklist_date, item))`);
  await run(`CREATE TABLE IF NOT EXISTS marketing_weekly_reports (id INTEGER PRIMARY KEY AUTOINCREMENT, week_start TEXT NOT NULL UNIQUE, leads_week INTEGER NOT NULL DEFAULT 0, budgets_week INTEGER NOT NULL DEFAULT 0, lead_origins TEXT, strong_campaigns TEXT, weak_campaigns TEXT, team_pending TEXT, suggested_next_steps TEXT, created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP)`);
  await run(`CREATE TABLE IF NOT EXISTS marketing_origin_performance (id INTEGER PRIMARY KEY AUTOINCREMENT, month TEXT NOT NULL, origin TEXT NOT NULL, leads INTEGER NOT NULL DEFAULT 0, qualified_leads INTEGER NOT NULL DEFAULT 0, budgets INTEGER NOT NULL DEFAULT 0, cost REAL NOT NULL DEFAULT 0, cost_per_lead REAL NOT NULL DEFAULT 0, cost_per_budget REAL NOT NULL DEFAULT 0, UNIQUE(month, origin))`);
  await run(`CREATE TABLE IF NOT EXISTS marketing_budget_targets (id INTEGER PRIMARY KEY AUTOINCREMENT, month TEXT NOT NULL UNIQUE, target_budgets INTEGER NOT NULL DEFAULT 15, generated_budgets INTEGER NOT NULL DEFAULT 0, status TEXT NOT NULL DEFAULT 'abaixo da meta')`);
  await run(`CREATE TABLE IF NOT EXISTS marketing_campaign_alerts (id INTEGER PRIMARY KEY AUTOINCREMENT, alert_type TEXT NOT NULL, title TEXT NOT NULL, message TEXT, severity TEXT NOT NULL DEFAULT 'Atenção', status TEXT NOT NULL DEFAULT 'Aberto', created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP)`);
  const month = new Date().toISOString().slice(0, 7); const today = new Date().toISOString().slice(0, 10);
  await run('INSERT OR IGNORE INTO marketing_goals (month, monthly_budget_target, notes) VALUES (?, ?, ?)', [month, 15, 'Meta inicial BELLEART: 15 orçamentos por mês.']);
  await run('INSERT OR IGNORE INTO marketing_budget_targets (month, target_budgets, status) VALUES (?, ?, ?)', [month, 15, 'abaixo da meta']);
  await run('INSERT OR IGNORE INTO flyer_campaigns (code,name,neighborhood,region,quantity_planned,responsible,cost,treatment,offer,whatsapp_message,qr_code_payload,short_link) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)', ['IMPLANTE-JDANGELA-JULHO','Panfleto Implante Jardim Ângela','Jardim Ângela','Ruas próximas à clínica',1000,'Equipe comercial',350,'Implante','Avaliação personalizada','Olá, vi o panfleto do implante e gostaria de saber mais.','wa.me/BELLEART?text=vi%20o%20panfleto%20do%20implante','belleart.link/implante-jdangela']);
  const flyer = await get('SELECT id FROM flyer_campaigns WHERE code=?', ['IMPLANTE-JDANGELA-JULHO']);
  await run('INSERT OR IGNORE INTO flyer_distribution (campaign_id,neighborhood,region,street,quantity_distributed,distribution_date,responsible,cost,notes) VALUES (?,?,?,?,?,?,?,?,?)', [flyer?.id,'Jardim Ângela','Comércio local','Av. principal',300,today,'Equipe comercial',120,'Distribuição inicial rastreável.']);
  for (const origin of ['Instagram','TikTok','Facebook','WhatsApp','Panfleto','Indicação','Google','Site','Tráfego pago']) await run('INSERT OR IGNORE INTO marketing_origin_performance (month,origin) VALUES (?,?)',[month,origin]);
  const leads = [['Ana Paula','11911111111','Implante','Panfleto','IMPLANTE-JDANGELA-JULHO','Alta'],['Carlos Lima','11922222222','Prótese protocolo','Instagram','Instagram Implantes','Alta'],['Marina Souza','11933333333','Ortodontia','TikTok','TikTok Prótese','Média'],['João Pedro','11944444444','Implante','WhatsApp','WA-AVALIACAO','Média'],['Lucia Alves','11955555555','Prótese','Indicação','Indicação paciente','Alta'],['Rita Gomes','11966666666','Estética','Google','Busca local','Baixa'],['Felipe Costa','11977777777','Implante','Facebook','Meta orgânico','Média'],['Bianca Reis','11988888888','Ortodontia','Site','Formulário site','Média'],['Sonia Nunes','11999999999','Dentadura','Panfleto','IMPLANTE-JDANGELA-JULHO','Alta'],['Mateus Rocha','11888888888','Implante','Tráfego pago','Instagram Implantes','Alta']];
  for (const l of leads) await run('INSERT OR IGNORE INTO lead_capture_entries (name,phone_whatsapp,interest,origin,campaign,urgency,best_contact_time,suggested_whatsapp_message,next_contact_date) VALUES (?,?,?,?,?,?,?,?,?)',[...l,'Manhã',`Olá, ${l[0]}! Aqui é da BELLEART. Podemos agendar sua avaliação de ${l[2]}?`,today]);
  const items = ['postar Reel','postar Stories','responder comentários','responder direct','registrar leads','ligar para leads','revisar follow-ups'];
  for (const item of items) await run('INSERT OR IGNORE INTO marketing_daily_checklist (checklist_date,item,planned_quantity) VALUES (?,?,?)',[today,item,item==='postar Stories'?3:1]);
  for (let i=0;i<10;i+=1) await run('INSERT OR IGNORE INTO commercial_daily_tasks (lead_id,title,channel,due_date,priority,message) VALUES (?,?,?,?,?,?)',[i+1,`Follow-up comercial ${i+1}`,i%3===0?'Telefone':'WhatsApp',today,i%2?'Média':'Alta','Mensagem pronta de conversão BELLEART.']);
  const templates = [
    ['Implante primeiro contato','Implante','curta','Olá! Vi seu interesse em implante. Vamos agendar uma avaliação?'],['Implante valor/condição','Implante','humanizada','O valor depende do seu caso. Na avaliação o Dr. Rogério explica opções e condições.'],['Implante falta de dinheiro','Implante','firme','Podemos avaliar formas facilitadas sem você adiar sua saúde bucal.'],['Implante medo','Implante','humanizada','É normal ter medo. A avaliação serve para explicar tudo com calma.'],['Orçamento sem resposta','Implante','firme','Posso reservar um horário para tirar suas dúvidas e avançarmos com segurança?'],['Convite avaliação','Implante','curta','Tenho horários para avaliação esta semana. Qual período fica melhor?'],['Ortodontia primeiro contato','Ortodontia','curta','Olá! Quer saber sobre aparelho/ortodontia? Vamos avaliar seu sorriso.'],['Ortodontia manutenção','Ortodontia','humanizada','A manutenção mantém o tratamento evoluindo com segurança.'],['Prótese protocolo','Prótese','humanizada','A protocolo pode melhorar mastigação e segurança; precisa de avaliação.'],['Panfleto vi o panfleto','Panfletagem','curta','Olá! Vi o panfleto e quero saber sobre implante. Podemos te explicar e agendar avaliação?']];
  for (const t of templates) await run('INSERT OR IGNORE INTO marketing_whatsapp_templates (title,category,message) VALUES (?,?,?)',[t[0],t[1],`Versão ${t[2]}: ${t[3]} CTA: agendar avaliação.`]);
  await run('INSERT OR IGNORE INTO marketing_weekly_reports (week_start,leads_week,budgets_week,lead_origins,strong_campaigns,weak_campaigns,team_pending,suggested_next_steps) VALUES (?,?,?,?,?,?,?,?)',[today,10,2,'Instagram, TikTok, Panfleto','IMPLANTE-JDANGELA-JULHO','Campanhas sem retorno em 7 dias','Confirmar avaliações e orçamentos sem resposta','Manter follow-up diário e reforçar Reels de implantes.']);
  const alerts = [['meta_mensal','Meta mensal abaixo do ritmo','Acompanhar diariamente até chegar aos 15 orçamentos.'],['panfleto_7_dias','Panfleto sem retorno em 7 dias','Revisar bairro, oferta e mensagem pronta.'],['lead_24h','Lead sem contato por 24h','Priorizar contato via WhatsApp e telefone.']];
  for (const a of alerts) await run('INSERT OR IGNORE INTO marketing_campaign_alerts (alert_type,title,message) VALUES (?,?,?)', a);
}
